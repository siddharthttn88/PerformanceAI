...
```
PerformanceAI/
├── 📂 src/           (22 files) - Source code modules
...
├── 📂 Reports/       (2 files)  - Test result reports (HTML + PDF)
...
- `html-to-pdf.js` - **⭐ Convert HTML report to PDF (run before emailing)**
...
### Reports/ - Test Reports (2 files)

- `result.html` - Latest Locust test report (raw HTML)
- `result.pdf` - PDF export of latest report (generated by `html-to-pdf.js`)
...
2. Convert to PDF: [src/utils/html-to-pdf.js](../src/utils/html-to-pdf.js) → `Reports/result.pdf`
3. Run [src/reporting/send-email-report.js](../src/reporting/send-email-report.js) with `--attach Reports/result.pdf`
4. Email sent to addresses in [config/stakeholder-email.csv](../config/stakeholder-email.csv)
